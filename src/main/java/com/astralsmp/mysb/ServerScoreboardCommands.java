...
public class ServerScoreboardCommands {
                        ...
                        .then(CommandManager.literal("add")
                                .then(CommandManager.argument("id", StringArgumentType.word())
                                        .suggests((context, builder) -> {
                                            // IDのサジェスト
                                            builder.suggest("my_custom_stat", new LiteralText("カスタム統計のID（英数字）"));
                                        ...
                                        .then(CommandManager.argument("displayName", StringArgumentType.string())
                                                .suggests((context, builder) -> {
                                                    ...
                                                    builder.suggest("\"My Custom Stat\"", new LiteralText("表示名（スペースを含む場合は\"\"で囲む）"));
                                                    return builder.buildFuture();
                                                })
        ...
        try {
            ...
            if (source.getEntity() instanceof ServerPlayerEntity player) {
                ...
                if (!RateLimiter.canPerformAction(player.getUuid(), "gui", ServerScoreboardConfig.GUI_OPEN_COOLDOWN_MS)) {
                    source.sendError(new LiteralText("コマンドを実行するには少し待ってください"));
                    return 0;
                }
                ...
                source.sendError(new LiteralText("このコマンドはプレイヤーのみ実行できます"));
                return 0;
            }
        } catch (Exception e) {
            ServerScoreboardLogger.error("Error executing admin GUI command", e);
            context.getSource().sendError(new LiteralText("コマンド実行中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
        ...
        try {
            ...
            if (source.getEntity() instanceof ServerPlayerEntity player) {
                if (!RateLimiter.canPerformAction(player.getUuid(), "reload", ServerScoreboardConfig.COMMAND_COOLDOWN_MS * 10)) {
                    source.sendError(new LiteralText("リロードコマンドを実行するには少し待ってください"));
                    return 0;
                }
            }
            ...
            context.getSource().sendFeedback(
                    new LiteralText("スコアボードデータを再読み込みしました"),
                    false
            );
            ...
            context.getSource().sendError(new LiteralText("スコアボードデータの再読み込み中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int addTotalStat(CommandContext<ServerCommandSource> context) {
        try {
            ...
            context.getSource().sendFeedback(
                new LiteralText("トータル統計「" + displayName + "」を追加しました"),
                true
            );
            ...
            context.getSource().sendError(new LiteralText("トータル統計の追加中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int listTotalStats(CommandContext<ServerCommandSource> context) {
        try {
            ...
            if (objectives.isEmpty()) {
                context.getSource().sendFeedback(new LiteralText("トータル統計が登録されていません"), false);
                return 1;
            }
            
            context.getSource().sendFeedback(new LiteralText("=== トータル統計一覧 ==="), false);
            for (String objName : objectives) {
                String displayName = TotalStatsManager.getTotalDisplayName(objName);
                context.getSource().sendFeedback(
                    new LiteralText("- " + objName + " (" + displayName + ")"),
                    false
                );
            }
            ...
            context.getSource().sendError(new LiteralText("トータル統計の一覧表示中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int updateTotalStats(CommandContext<ServerCommandSource> context) {
        try {
            ...
            context.getSource().sendFeedback(
                new LiteralText("全てのトータル統計を更新しました"),
                true
            );
            ...
            context.getSource().sendError(new LiteralText("トータル統計の更新中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static CompletableFuture<Suggestions> suggestAvailableStats(CommandContext<ServerCommandSource> context, SuggestionsBuilder builder) {
        ...
        for (Map.Entry<String, String> entry : availableStats.entrySet()) {
            ...
            if (!enabledStats.contains(statId)) {
                builder.suggest(statId, new LiteralText(displayName));
            }
        }
    ...
    private static int enableStat(CommandContext<ServerCommandSource> context) {
        try {
            ...
            if (!TotalStatsManager.getAllAvailableStats().containsKey(statId)) {
                context.getSource().sendError(new LiteralText("不明な統計: " + statId));
                return 0;
            }
            ...
            context.getSource().sendFeedback(
                new LiteralText("統計を有効化しました: " + statId),
                true
            );
            ...
            context.getSource().sendError(new LiteralText("統計の有効化中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int disableStat(CommandContext<ServerCommandSource> context) {
        try {
            ...
            context.getSource().sendFeedback(
                new LiteralText("統計を無効化しました: " + statId),
                true
            );
            ...
            context.getSource().sendError(new LiteralText("統計の無効化中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int listStatStatus(CommandContext<ServerCommandSource> context) {
        try {
            ...
            context.getSource().sendFeedback(new LiteralText("=== 統計の状態 ==="), false);
            
            context.getSource().sendFeedback(new LiteralText("有効:").formatted(Formatting.GREEN), false);
            for (String statId : enabledStats) {
                String displayName = allStats.getOrDefault(statId, statId);
                context.getSource().sendFeedback(
                    new LiteralText("  - " + statId + " (" + displayName + ")").formatted(Formatting.GREEN),
                    false
                );
            }
            ...
            for (Map.Entry<String, String> entry : allStats.entrySet()) {
                if (!enabledStats.contains(entry.getKey())) {
                    context.getSource().sendFeedback(
                        new LiteralText("  - " + entry.getKey() + " (" + entry.getValue() + ")").formatted(Formatting.RED),
                        false
                    );
                }
            }
            ...
            context.getSource().sendError(new LiteralText("統計の一覧表示中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    
    private static int showTotalHelp(CommandContext<ServerCommandSource> context) {
        context.getSource().sendFeedback(new LiteralText("=== トータル統計コマンドの使い方 ===").formatted(Formatting.GOLD), false);
        context.getSource().sendFeedback(new LiteralText(""), false);
        context.getSource().sendFeedback(new LiteralText("/mysb total add <id> <displayName> <statType>").formatted(Formatting.YELLOW), false);
        context.getSource().sendFeedback(new LiteralText("  新しいトータル統計を追加").formatted(Formatting.GRAY), false);
        context.getSource().sendFeedback(new LiteralText("  例: /mysb total add damage \"Total Damage Dealt\" damage_dealt").formatted(Formatting.DARK_GRAY), false);
        context.getSource().sendFeedback(new LiteralText(""), false);
        context.getSource().sendFeedback(new LiteralText("/mysb total list").formatted(Formatting.YELLOW), false);
        context.getSource().sendFeedback(new LiteralText("  登録されているトータル統計を一覧表示").formatted(Formatting.GRAY), false);
        context.getSource().sendFeedback(new LiteralText(""), false);
        context.getSource().sendFeedback(new LiteralText("/mysb total update").formatted(Formatting.YELLOW), false);
        context.getSource().sendFeedback(new LiteralText("  トータル統計を手動更新").formatted(Formatting.GRAY), false);
        context.getSource().sendFeedback(new LiteralText(""), false);
        context.getSource().sendFeedback(new LiteralText("/mysb total remove <id>").formatted(Formatting.YELLOW), false);
        context.getSource().sendFeedback(new LiteralText("  カスタムトータル統計を削除").formatted(Formatting.GRAY), false);
        context.getSource().sendFeedback(new LiteralText(""), false);
        context.getSource().sendFeedback(new LiteralText("利用可能な統計タイプ:").formatted(Formatting.AQUA), false);
        context.getSource().sendFeedback(new LiteralText("  mined, placed, killed, deaths, damage_dealt,").formatted(Formatting.DARK_AQUA), false);
        context.getSource().sendFeedback(new LiteralText("  damage_taken, play_time, walk_one_cm, jump, fish_caught").formatted(Formatting.DARK_AQUA), false);
        return 1;
    }
    ...
    private static CompletableFuture<Suggestions> suggestStatTypes(CommandContext<ServerCommandSource> context, SuggestionsBuilder builder) {
        ...
        for (Map.Entry<String, String> entry : commonStats.entrySet()) {
            builder.suggest(entry.getKey(), new LiteralText(entry.getValue()));
        }
        return builder.buildFuture();
    }
    ...
    private static CompletableFuture<Suggestions> suggestCustomStats(CommandContext<ServerCommandSource> context, SuggestionsBuilder builder) {
        ...
        for (String statId : allStats.keySet()) {
            ...
            if (!statId.equals("mined") && !statId.equals("placed") && 
                ...
                builder.suggest(statId, new LiteralText(allStats.get(statId)));
            }
        }
        return builder.buildFuture();
    }
    ...
    private static int removeTotalStat(CommandContext<ServerCommandSource> context) {
        try {
            ...
            if (id.equals("mined") || id.equals("placed") || id.equals("killed") || id.equals("deaths")) {
                context.getSource().sendError(new LiteralText("デフォルト統計は削除できません"));
                return 0;
            }
            ...
            context.getSource().sendFeedback(
                new LiteralText("トータル統計「" + id + "」を削除しました"),
                true
            );
            ...
            context.getSource().sendError(new LiteralText("トータル統計の削除中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int excludePlayer(CommandContext<ServerCommandSource> context) {
        try {
            ...
            if (TotalStatsManager.isPlayerExcluded(playerName)) {
                context.getSource().sendError(new LiteralText("プレイヤー " + playerName + " は既に除外されています"));
                return 0;
            }
            ...
            context.getSource().sendFeedback(
                new LiteralText("プレイヤー " + playerName + " を統計から除外しました"),
                true
            );
            ...
            context.getSource().sendError(new LiteralText("プレイヤーの除外中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int includePlayer(CommandContext<ServerCommandSource> context) {
        try {
            ...
            if (!TotalStatsManager.isPlayerExcluded(playerName)) {
                context.getSource().sendError(new LiteralText("プレイヤー " + playerName + " は除外されていません"));
                return 0;
            }
            ...
            context.getSource().sendFeedback(
                new LiteralText("プレイヤー " + playerName + " を統計に含めるようにしました"),
                true
            );
            ...
            context.getSource().sendError(new LiteralText("プレイヤーの包含中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int listExcludedPlayers(CommandContext<ServerCommandSource> context) {
        try {
            ...
            if (excludedPlayers.isEmpty()) {
                context.getSource().sendFeedback(new LiteralText("除外されているプレイヤーはいません"), false);
                return 1;
            }
            
            context.getSource().sendFeedback(new LiteralText("=== 除外されているプレイヤー ===").formatted(Formatting.GOLD), false);
            for (String playerName : excludedPlayers) {
                context.getSource().sendFeedback(
                    new LiteralText("- " + playerName).formatted(Formatting.YELLOW),
                    false
                );
            }
            ...
            context.getSource().sendError(new LiteralText("除外プレイヤーリストの表示中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int showVersion(CommandContext<ServerCommandSource> context) {
        context.getSource().sendFeedback(
            new LiteralText("MySB - My Scoreboard").formatted(Formatting.GOLD)
                .append(new LiteralText(" Version: ").formatted(Formatting.GRAY))
                .append(new LiteralText(ServerOnlyScoreboardMod.getModVersion()).formatted(Formatting.AQUA)),
            false
        );
        return 1;
    }
    ...
    private static int enableFakePlayerScore(CommandContext<ServerCommandSource> context) {
        try {
            ...
            context.getSource().sendFeedback(
                new LiteralText("Fake Player のスコア表示を有効にしました").formatted(Formatting.GREEN),
                true
            );
            ...
            context.getSource().sendError(new LiteralText("Fake Player スコア有効化中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int disableFakePlayerScore(CommandContext<ServerCommandSource> context) {
        try {
            ...
            context.getSource().sendFeedback(
                new LiteralText("Fake Player のスコア表示を無効にしました").formatted(Formatting.YELLOW),
                true
            );
            ...
            context.getSource().sendError(new LiteralText("Fake Player スコア無効化中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int openDiscordGUI(CommandContext<ServerCommandSource> context) {
        try {
            ...
            context.getSource().sendError(new LiteralText("Discord GUI を開く際にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int setDiscordChannel(CommandContext<ServerCommandSource> context) {
        try {
            ...
            if (!channelId.matches("\\d+")) {
                context.getSource().sendError(new LiteralText("無効なチャンネルID: 数字のみを入力してください"));
                return 0;
            }
            ...
            context.getSource().sendFeedback(
                new LiteralText("Discordフォーラムチャンネルを設定しました: " + channelId).formatted(Formatting.GREEN),
                true
            );
            ...
            context.getSource().sendError(new LiteralText("チャンネル設定中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int showDiscordStatus(CommandContext<ServerCommandSource> context) {
        try {
            ...
            source.sendFeedback(new LiteralText("=== Discord連携ステータス ===").formatted(Formatting.GOLD), false);

            // Token設定状態
            boolean hasToken = DiscordConfig.hasValidToken();
            source.sendFeedback(
                new LiteralText("Bot Token: ").formatted(Formatting.GRAY)
                    .append(new LiteralText(hasToken ? "設定済み" : "未設定").formatted(hasToken ? Formatting.GREEN : Formatting.RED)),
                false
            );
            ...
            source.sendFeedback(
                new LiteralText("フォーラムチャンネル: ").formatted(Formatting.GRAY)
                    .append(new LiteralText(hasChannel ? DiscordConfig.getForumChannelId() : "未設定")
            ...
            source.sendFeedback(
                new LiteralText("接続状態: ").formatted(Formatting.GRAY)
                    .append(new LiteralText(connected ? "接続中" : "未接続").formatted(connected ? Formatting.GREEN : Formatting.RED)),
                false
            );
            ...
            source.sendFeedback(
                new LiteralText("Discord有効統計: ").formatted(Formatting.GRAY)
                    .append(new LiteralText(enabledStats.isEmpty() ? "なし" : String.join(", ", enabledStats))
            ...
            if (connected && !enabledStats.isEmpty()) {
                source.sendFeedback(
                    new LiteralText("次回更新まで: ").formatted(Formatting.GRAY)
                        .append(new LiteralText(DiscordScheduler.getTimeUntilNextUpdateFormatted()).formatted(Formatting.AQUA)),
                    false
                );
            }
            ...
            context.getSource().sendError(new LiteralText("ステータス表示中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int reconnectDiscord(CommandContext<ServerCommandSource> context) {
        try {
            if (!DiscordConfig.hasValidToken()) {
                context.getSource().sendError(new LiteralText("Bot Tokenが設定されていません"));
                return 0;
            }

            context.getSource().sendFeedback(
                new LiteralText("Discord接続をテスト中...").formatted(Formatting.YELLOW),
                false
            );
            ...
            DiscordManager.testConnection().thenAccept(success -> {
                if (success) {
                    context.getSource().sendFeedback(
                        new LiteralText("Discord接続に成功しました").formatted(Formatting.GREEN),
                        true
                    );
                } else {
                    context.getSource().sendError(new LiteralText("Discord接続に失敗しました"));
                }
            });
            ...
            context.getSource().sendError(new LiteralText("再接続中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int testDiscord(CommandContext<ServerCommandSource> context) {
        try {
            ...
            if (!DiscordConfig.hasForumChannelId()) {
                context.getSource().sendError(new LiteralText("フォーラムチャンネルIDが設定されていません"));
                return 0;
            }

            context.getSource().sendFeedback(
                new LiteralText("テスト投稿を実行中...").formatted(Formatting.YELLOW),
                false
            );
            ...
            DiscordStatsPublisher.testPublish().thenAccept(success -> {
                if (success) {
                    context.getSource().sendFeedback(
                        new LiteralText("テスト投稿に成功しました").formatted(Formatting.GREEN),
                        true
                    );
                } else {
                    context.getSource().sendError(new LiteralText("テスト投稿に失敗しました"));
                }
            });
            ...
            context.getSource().sendError(new LiteralText("テスト投稿中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int forceDiscordUpdate(CommandContext<ServerCommandSource> context) {
        try {
            ...
            if (enabledStats.isEmpty()) {
                context.getSource().sendError(new LiteralText("Discord有効な統計がありません"));
                return 0;
            }

            if (DiscordScheduler.isUpdating()) {
                context.getSource().sendError(new LiteralText("更新処理が既に実行中です"));
                return 0;
            }

            context.getSource().sendFeedback(
                new LiteralText("Discord統計を更新中...").formatted(Formatting.YELLOW),
                false
            );

            DiscordScheduler.forceUpdate().thenRun(() -> {
                context.getSource().sendFeedback(
                    new LiteralText("Discord統計の更新が完了しました").formatted(Formatting.GREEN),
                    true
                );
            });
            ...
            context.getSource().sendError(new LiteralText("更新中にエラーが発生しました: " + e.getMessage()));
            return 0;
        }
    }
}