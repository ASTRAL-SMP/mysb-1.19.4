...
public class ServerScoreboardCommands {
                ...
                .then(CommandManager.literal("discord")
                        ...
                        .then(CommandManager.literal("add")
                                ...
                                .then(CommandManager.argument("objective", StringArgumentType.word())
                                        .suggests(ServerScoreboardCommands::suggestAvailableObjectives)
                                        .executes(ServerScoreboardCommands::addDiscordObjective)))
                        .then(CommandManager.literal("remove")
                                .requires(source -> source.hasPermissionLevel(0)) // 権限レベル0
                                .then(CommandManager.argument("objective", StringArgumentType.word())
                                        .suggests(ServerScoreboardCommands::suggestActiveDiscordObjectives)
    ...
    private static int addDiscordObjective(CommandContext<ServerCommandSource> context) {
        ...
        // 有効な統計オブジェクティブかチェック
        List<String> enabledObjectives = TotalStatsManager.getEnabledTotalObjectives();
        if (!enabledObjectives.contains(objective)) {
            context.getSource().sendError(Text.literal("指定されたオブジェクティブは利用できません: " + objective));
            context.getSource().sendError(Text.literal("利用可能な統計: " + String.join(", ", enabledObjectives)));
            return 0;
        }
        
        // 既にDiscord連携されているかチェック
        if (SimpleDiscordBot.getInstance().getForumThreads().containsKey(objective)) {
            context.getSource().sendError(Text.literal("このオブジェクティブは既にDiscord連携されています: " + objective));
            return 0;
        }
    ...
    private static CompletableFuture<Suggestions> suggestDiscordObjectives(CommandContext<ServerCommandSource> context, SuggestionsBuilder builder) {
        // 実際に存在するスコアボードオブジェクティブを提案
        ServerCommandSource source = context.getSource();
        if (source.getServer() != null) {
            var scoreboard = source.getServer().getScoreboard();
            for (var objective : scoreboard.getObjectives()) {
                String name = objective.getName();
                if (name.startsWith(builder.getRemaining().toLowerCase()) || builder.getRemaining().isEmpty()) {
    ...
    private static CompletableFuture<Suggestions> suggestAvailableObjectives(CommandContext<ServerCommandSource> context, SuggestionsBuilder builder) {
        // 指定された8つの統計のみ提案（現在Discord投稿中でないもの）
        var activeDiscordObjectives = SimpleDiscordBot.getInstance().getForumThreads().keySet();
        
        // TotalStatsManagerから有効な統計オブジェクティブを取得
        List<String> enabledObjectives = TotalStatsManager.getEnabledTotalObjectives();
        
        for (String objectiveName : enabledObjectives) {
            // Discord投稿中でないもののみ提案
            if (!activeDiscordObjectives.contains(objectiveName)) {
                if (objectiveName.toLowerCase().startsWith(builder.getRemaining().toLowerCase()) || builder.getRemaining().isEmpty()) {
                    builder.suggest(objectiveName);
                }
            }
        }
        return builder.buildFuture();
    ...
    private static CompletableFuture<Suggestions> suggestActiveDiscordObjectives(CommandContext<ServerCommandSource> context, SuggestionsBuilder builder) {
        // 現在Discord投稿中のオブジェクティブのみ提案
        for (String objective : SimpleDiscordBot.getInstance().getForumThreads().keySet()) {
            if (objective.startsWith(builder.getRemaining().toLowerCase()) || builder.getRemaining().isEmpty()) {
